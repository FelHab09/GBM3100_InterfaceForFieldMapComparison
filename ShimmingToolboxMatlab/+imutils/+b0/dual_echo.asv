...
mag_data = abs(complVol); %FH: Takes magnitude=sqrt(a^2+b^2) in z=a+bi for each element of matrix complVol
ph_data = angle(complVol); %FH: Takes phase=theta in z=abs(z).*exp(i*theta) for each element of matrix complVol

% ph_data difference reconstruction: Handbook of MRI pulse sequences S.13.5.1
